...
<b> *The following examples check if "$PWD" <br> (a command whis prints the name of the current directory that the terminal you are using is doing stuff from or to) is a folder that exists, and then outputs a text chosen by you if it is or if it isn't (hopefully is)* </b>
